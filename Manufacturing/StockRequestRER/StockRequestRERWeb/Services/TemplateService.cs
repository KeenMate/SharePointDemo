	...
	public class TemplateService
	{
		...
		public string CreateMessage(TemplateModel template)
		{
			...
			foreach (string s in template.TemplatePaths)
			{
				...
				{
					logger.Warn("Can not add template " + Path.GetFileNameWithoutExtension(s) + ". Template probably added.");
				}
			}
			...
			foreach (string s in template.TemplatePaths)
			{
				...
				{
					logger.Debug("Compiling subtemplates " + Path.GetFileNameWithoutExtension(s));
