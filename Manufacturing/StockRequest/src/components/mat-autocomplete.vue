...
export default {
  ...
  watch: {
    searchText: debounce(function(value) {
      var self = this;
      service.searchByTitle(value).done(function(result) {
        $("#autocomplete-input").off();
        $("#autocomplete-input").autocomplete({
          data: service.parseAutocompleteData(result),
          limit: Config.StockRequest.AutocompleteLimit,
  ...
  methods: {
    autocomplete: function(params, result) {
      ...
      service.loadStockItemWithMaterialType(params).done(function(stockAmount) {
