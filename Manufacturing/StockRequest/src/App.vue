  ...
  <div id="app">
    ...
    <div class="row">
      <button :disabled="!ButtonEnabled" class="btn waves-effect waves-light" @click.prevent="send" type="submit" name="action">
        ...
        <span v-else-if="stockItems.length === 1">Request 1 item</span>
        <span v-else>Request {{stockItems.length}} items</span>
      </button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    testBtn: function(lock = false) {
      ...
      if (
        this.stockItems.filter(function(e) {
          return e.Amount > 0;
        }).length === this.stockItems.length &&
      ...
      setTimeout(function() {
        var fn = function() {
          if (
            ...
            clearInterval(x);
            self.DeliveredOn = moment($("#datepicker").val()).utcOffset(
              "+0200"
            );
            if ($("#datepicker").val() === "") self.TimeDisabled = true;
            else self.TimeDisabled = false;
    ...
    parseTimePicker: function(fast) {
      var self = this;
      if (fast === true) {
        self.DeliveredOn = moment(
          moment(self.DeliveredOn).format("YYYY-MM-DD") +
            " " +
            $("#timepicker").val()
        ).utcOffset("+0200");
        return;
    ...
    addItem: function(item) {
      ...
      if (this.DateDisabled === true) this.initializePicker();
